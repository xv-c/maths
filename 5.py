...
for idx in range(1, m + 1):
    ...
    avg_g = g_sum / n
    avg_h = h_sum / n
    avg_gs.append(avg_g)  # Добавление среднего g в список
    avg_hs.append(avg_h)  # Добавление среднего h в список
    ...
    std_g = disp_g ** 0.5
    std_h = disp_h ** 0.5
    std_gs.append(std_g)  # Добавление стандартного отклонения g в список
    std_hs.append(std_h)  # Добавление стандартного отклонения h в список
...
t = 2.58  # t-значение для 99% доверительного интервала
...
print(f"{min_conf_g} < mg < {max_conf_g}")  # Вывод доверительного интервала для g
...
# Для величины h
epsilon_h = t * std_hs[0] / n ** 0.5  # Ошибка эпсилон для h
...
print(f"{min_conf_h} < mh < {max_conf_h}")  # Вывод доверительного интервала для h
